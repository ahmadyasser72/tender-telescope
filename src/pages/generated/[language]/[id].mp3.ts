...
type Props = Pick<Question, "code" | "sourceWord" | "language">;
...
export const getStaticPaths = (async () => {
  ...
  return allQuestions.map(({ id, language, code, sourceWord }) => ({
    params: { language, id },
    props: { code, sourceWord, language } satisfies Props,
  }));
...
export const GET: APIRoute = async (context) => {
  ...
  const { code, sourceWord, language } = context.props as Props;

  return upstash.get(`${language}-${id}-google-translate-tts`, async () => {
    const urls = [
